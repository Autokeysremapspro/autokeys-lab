...
async function attachRelations(expedientes: Expediente[]): Promise<ExpedienteConRelaciones[]> {
  ...
  const clientes = new Map((clientesRes.data || []).map((c: Cliente) => [c.id, c]))
  const vehiculos = new Map((vehiculosRes.data || []).map((v: Vehiculo) => [v.id, v]))
  const ecus = new Map((ecuRes.data || []).map((e: ExpedienteECU) => [e.expediente_id, e]))
  const llaves = new Map((llavesRes.data || []).map((l: ExpedienteLlaves) => [l.expediente_id, l]))
