...
`)
  ...
  async function deleteVehiculo(vehiculo: Vehiculo) {
    ...
    if ((count || 0) > 0) {
      toast.error('No se puede eliminar: tiene expedientes asociados.')
      return
    }

    const title = [vehiculo.marca, vehiculo.modelo, vehiculo.matricula].filter(Boolean).join(' ')
    if (!confirm(`¿Eliminar definitivamente ${title || 'este vehículo'}?`)) return
      ...
      <div className="mb-5 grid gap-4 xl:grid-cols-[1fr_180px_180px_180px]">
        <div className="card flex items-center gap-3 p-4">
          ...
          <input
            ...
            placeholder="Buscar matrícula, VIN, cliente, motor, ECU, HW, SW..."
            className="w-full border-0 bg-transparent p-0"
          />
        </div>
        <div className="card p-4">
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Vehículos</p>
          <p className="mt-1 text-2xl font-black">{vehiculos.length}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Con ECU</p>
          <p className="mt-1 text-2xl font-black">{conEcu}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Sin cliente</p>
          <p className="mt-1 text-2xl font-black">{sinCliente}</p>
        </div>
      </div>

      {loading ? (
        <div className="card p-8 text-zinc-500">Cargando vehículos...</div>
      ) : filtered.length === 0 ? (
        <div className="card p-8 text-zinc-500">No hay vehículos que coincidan con la búsqueda.</div>
      ) : (
              ...
              <div key={vehiculo.id} className="card p-5 transition hover:-translate-y-0.5 hover:border-red-500/35">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-red-400">
                      <Gauge size={14} /> {vehiculo.matricula || 'Sin matrícula'}
                    </div>
                    <h3 className="mt-2 text-xl font-black">{title}</h3>
                    <p className="mt-1 text-sm text-zinc-500">{vehiculo.motor || 'Motor sin definir'} · {vehiculo.anio || 'Año —'}</p>
                  </div>
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-red-600/15 text-red-400">
                    <Car size={24} />
                  </div>
                </div>
                ...
                <div className="mt-5 grid gap-3 text-sm">
                  <div className="rounded-2xl bg-black/20 p-3">
                    <div className="flex items-center gap-2 text-zinc-500"><User size={15} /> Cliente</div>
                    <div className="mt-1 font-bold text-zinc-200">{vehiculo.clientes?.nombre || 'Sin cliente asignado'}</div>
                  </div>
                  <div className="rounded-2xl bg-black/20 p-3">
                    <div className="flex items-center gap-2 text-zinc-500"><FileText size={15} /> VIN / ECU</div>
                    <div className="mt-1 truncate font-bold text-zinc-200">{vehiculo.bastidor || 'VIN —'} · {vehiculo.ecu || 'ECU —'}</div>
                  </div>
                </div>
