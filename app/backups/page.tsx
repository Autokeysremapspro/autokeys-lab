...
export default function BackupsPage() {
  ...
  const [selected, setSelected] = useState<BackupTable[]>(
  ...
  async function load() {
    ...
    try {
      const data = await getBackupRegistros()
      setRegistros(data as BackupRegistro[])
    } catch (err: any) {
  ...
  const totalRows = useMemo(
    () =>
      registros.reduce(
        (acc, item) => acc + Number(item.total_registros || 0),
        0
      ),
    [registros]
  )

  async function runQuickExport(table: BackupTable, label: string) {
    setWorking(`${table}-csv`)
    setError(null)
    setSuccess(null)

    try {
      await exportTable(table)
      setSuccess(`${label} exportado correctamente.`)
      await load()
    } catch (err: any) {
  ...
  async function runFullExport() {
    ...
    try {
      await exportFullJson(selected)
      setSuccess(`Copia completa descargada (${selected.length} tablas).`)
      await load()
    } catch (err: any) {
  ...
  function toggleTable(table: BackupTable) {
    setSelected((current) =>
  ...
  return (
    <AppShell>
      <div className="space-y-8">
        ...
        {success && (
          <div className="card p-4 border border-emerald-500/30 text-emerald-300">
            {success}
          </div>
        )}
            ...
            <div className="flex items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-zinc-500 mt-1">
                  Descarga CSV de las tablas principales para revisar en Excel
                  o guardar copia externa.
                </p>
              </div>
              ...
              {QUICK_EXPORTS.map((item) => (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      onClick={() => runQuickExport(item.table, item.label)}
                      disabled={!!working}
                      className="btn btn-red flex items-center gap-2 disabled:opacity-50"
                    >
                      {working === `${item.table}-csv` ? (
          ...
          <section className="card p-6">
            ...
            <div className="space-y-2 max-h-[430px] overflow-auto pr-1">
              {BACKUP_TABLES.map((table) => (
                <label
                  key={table.name}
                  className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 cursor-pointer hover:bg-white/[0.06]"
                >
                  <span>
                  ...
                  {registros.map((item) => (
                    <tr key={item.id} className="hover:bg-white/[0.03]">
                      ...
                      <td className="py-4 pr-4 text-zinc-400 max-w-[420px] truncate">
                        {(item.tablas || []).join(', ') || item.descripcion || '—'}
                      </td>
