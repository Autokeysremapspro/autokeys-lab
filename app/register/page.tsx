      ...
      <div className="relative mx-auto grid min-h-screen max-w-[1600px] lg:grid-cols-[.78fr_1.22fr]">
        <aside className="relative hidden overflow-hidden border-r border-white/10 lg:block">
          ...
          <div className="relative flex h-full flex-col justify-between p-10 xl:p-14">
            ...
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-[#c81f2a]/30 bg-[#c81f2a]/10 px-4 py-2 text-xs font-bold uppercase tracking-[.24em] text-[#ff5468]"><Sparkles size={15}/> Acceso profesional</span>
              <h1 className="mt-6 text-5xl font-bold uppercase italic leading-[.95] xl:text-7xl">Únete a la red de <span className="text-[#c81f2a]">Autokeys.</span></h1>
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              {['Cuenta verificada', 'Precios profesionales', 'Soporte técnico'].map((item) => <div key={item} className="rounded-2xl border border-white/10 bg-black/45 p-4 text-sm font-bold backdrop-blur-xl"><CheckCircle2 className="mb-3 text-[#ff5468]" size={20}/>{item}</div>)}
            </div>
          </div>
        </aside>
        ...
        <section className="relative flex items-center justify-center px-5 py-8 md:px-10 xl:px-16">
          <div className="w-full max-w-4xl rounded-[2rem] border border-white/12 bg-[#090b0f]/92 p-6 shadow-[0_50px_140px_rgba(0,0,0,.72)] backdrop-blur-2xl md:p-9">
            <div className="flex flex-col justify-between gap-5 border-b border-white/10 pb-6 sm:flex-row sm:items-center">
              <div>
                <p className="text-xs font-bold uppercase tracking-[.25em] text-[#ff5468]">Solicitud de distribuidor</p>
                <h2 className="mt-2 text-3xl font-bold uppercase md:text-4xl">{['Empresa','Contacto y acceso','Actividad profesional','Revisión final'][step-1]}</h2>
              </div>
              <div className="min-w-[180px]"><div className="flex justify-between text-xs font-bold text-white/40"><span>Paso {step}/4</span><span>{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-gradient-to-r from-[#7a0f16] to-[#ff5468] transition-all" style={{width:`${progress}%`}} /></div></div>
            </div>
            ...
            <div className="mt-7 min-h-[390px]">
              ...
              {step === 4 && <div className="grid gap-6 lg:grid-cols-[1fr_.75fr]"><div className="rounded-3xl border border-white/10 bg-white/[.035] p-6"><h3 className="text-xl font-bold uppercase">Resumen de solicitud</h3><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2">{[['Empresa',form.empresa],['Contacto',`${form.nombre} ${form.apellidos}`],['Email',form.email],['Teléfono',form.telefono],['Ubicación',[form.ciudad,form.provincia,form.pais].filter(Boolean).join(', ')],['Especialidad',form.especialidad],['Herramientas',form.herramientas.join(', ')||'No indicadas']].map(([k,v])=><div key={k}><dt className="text-xs font-bold uppercase tracking-wider text-white/35">{k}</dt><dd className="mt-1 font-bold text-white/80">{v}</dd></div>)}</dl></div><div className="space-y-4"><textarea value={form.observaciones} onChange={(e)=>field('observaciones',e.target.value)} placeholder="Cuéntanos brevemente qué tipo de trabajos realizas..." className="min-h-40 w-full rounded-2xl border border-white/12 bg-black/30 p-4 text-sm outline-none focus:border-[#c81f2a]/60"/><label className="flex cursor-pointer gap-3 rounded-2xl border border-white/10 bg-white/[.035] p-4 text-sm text-white/60"><input type="checkbox" checked={form.acepta} onChange={(e)=>field('acepta',e.target.checked)} className="mt-1 accent-[#c81f2a]"/><span>Acepto que Autokeys revise la solicitud y que el acceso no será efectivo hasta su aprobación.</span></label></div></div>}
            </div>

            <div className="mt-7 flex items-center justify-between gap-3 border-t border-white/10 pt-6">
              <button onClick={()=>step===1?router.push('/'):setStep(step-1)} className="inline-flex items-center gap-2 rounded-2xl border border-white/12 bg-white/[.035] px-5 py-3 text-sm font-bold uppercase text-white/65 hover:text-white"><ArrowLeft size={18}/>{step===1?'Volver':'Atrás'}</button>
              {step<4?<button onClick={next} className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-[#7a0f16] to-[#c81f2a] px-6 py-3 text-sm font-bold uppercase shadow-[0_16px_45px_rgba(217,4,41,.3)]">Continuar <ChevronRight size={18}/></button>:<button onClick={submit} disabled={loading} className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-[#7a0f16] to-[#c81f2a] px-7 py-3 text-sm font-bold uppercase disabled:opacity-60">{loading?'Enviando...':'Enviar solicitud'} <ArrowRight size={18}/></button>}
            </div>
            <p className="mt-5 text-center text-sm text-white/35">¿Ya tienes acceso? <Link href="/login" className="font-bold text-[#ff5468]">Iniciar sesión</Link></p>
          </div>
        </section>
      </div>
    </main>
...
function Input({label,icon:Icon,value,onChange,type='text',placeholder}:any){return <label className="block"><span className="mb-2 block text-[11px] font-bold uppercase tracking-[.2em] text-white/35">{label}</span><div className="flex h-14 items-center rounded-2xl border border-white/12 bg-black/30 focus-within:border-[#c81f2a]/60"><Icon className="ml-4 text-white/30" size={18}/><input type={type} value={value} onChange={(e)=>onChange(e.target.value)} placeholder={placeholder} className="h-full w-full bg-transparent px-4 text-sm outline-none placeholder:text-white/20"/></div></label>}
function Choice({title,items,selected,onToggle,single=false}:any){return <div><p className="mb-3 text-[11px] font-bold uppercase tracking-[.2em] text-[#0a0d12]/35">{title}</p><div className="flex flex-wrap gap-2">{items.map((item:string)=><button type="button" key={item} onClick={()=>onToggle(item)} className={`rounded-full border px-4 py-2 text-sm font-bold transition ${selected.includes(item)?'border-[#c81f2a] bg-[#c81f2a]/15 text-[#0a0d12]':'border-white/10 bg-white/[.035] text-[#0a0d12]/45 hover:text-[#0a0d12]'}`}>{item}</button>)}</div></div>}
