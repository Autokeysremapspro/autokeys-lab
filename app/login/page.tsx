  ...
  return (
    <main className="min-h-screen grid place-items-center p-4 bg-[radial-gradient(circle_at_top,#2a0004,#070707_45%)] text-zinc-100">
      <form onSubmit={submit} className="card w-full max-w-md p-8 space-y-5">
        <div>
          <p className="text-xs text-red-400 uppercase tracking-[0.24em] font-black">Acceso interno</p>
          <h1 className="text-4xl font-black mt-2">AUTOKEYS <span className="text-red-500">CORE</span></h1>
          <p className="text-zinc-500 mt-2">Laboratorio de electrónica · ECU · IMMO · Llaves</p>
        </div>

        <div className="space-y-3">
          <label className="text-sm font-bold text-zinc-300 flex items-center gap-2"><Mail size={16} /> Email</label>
          <input required type="email" className="w-full" placeholder="info@autokeyspro.es" value={email} onChange={e => setEmail(e.target.value)} />
        </div>

        <div className="space-y-3">
          <label className="text-sm font-bold text-zinc-300 flex items-center gap-2"><LockKeyhole size={16} /> Contraseña</label>
          <input required className="w-full" placeholder="Contraseña" type="password" value={password} onChange={e => setPassword(e.target.value)} />
        </div>
        ...
        <div className="rounded-2xl bg-emerald-500/5 border border-emerald-500/20 p-4 text-sm text-zinc-400 flex gap-3">
          ...
          <p>El acceso se gestiona con Supabase Auth. Los usuarios y roles se administran desde el apartado Usuarios.</p>
        </div>
      </form>
    </main>
  )
