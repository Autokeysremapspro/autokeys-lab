...
import { Camera, Car, CheckCircle2, ClipboardList, Loader2, Phone, Plus, User } from 'lucide-react'
...
export default function MobileQuickCreate() {
  ...
  const [trabajo, setTrabajo] = useState({ tipo_trabajo: 'Clonación ECU', prioridad: 'normal', descripcion: '', tecnico: 'Carlos' })
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <main className="mx-auto max-w-xl space-y-5 p-4 pb-28">
        {step === 1 && (
          <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="mb-5 flex items-center gap-3">
              <div className="rounded-2xl bg-red-600/15 p-3 text-red-400"><User /></div>
              <div><h2 className="text-2xl font-black">Cliente</h2><p className="text-sm text-slate-400">Datos mínimos para crear el expediente.</p></div>
            </div>
        ...
        {step === 2 && (
          <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="mb-5 flex items-center gap-3">
              <div className="rounded-2xl bg-red-600/15 p-3 text-red-400"><Car /></div>
              <div><h2 className="text-2xl font-black">Vehículo</h2><p className="text-sm text-slate-400">Pensado para rellenar rápido desde recepción o taller.</p></div>
            </div>
        ...
        {step === 3 && (
          <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="mb-5 flex items-center gap-3">
              <div className="rounded-2xl bg-red-600/15 p-3 text-red-400"><ClipboardList /></div>
              <div><h2 className="text-2xl font-black">Servicio</h2><p className="text-sm text-slate-400">Crea la OT con el tipo de trabajo correcto.</p></div>
            </div>
            ...
            <label className="block">
              ...
              <textarea value={trabajo.descripcion} onChange={(e) => setTrabajo({ ...trabajo, descripcion: e.target.value })} placeholder="Síntoma, lo que pide el cliente, observaciones..." className="h-32 w-full rounded-2xl border border-white/10 bg-slate-900 px-4 py-4 text-white outline-none focus:border-red-500" />
            </label>
          </section>
        )}
        ...
        {step === 4 && (
          <section className="rounded-3xl border border-emerald-500/25 bg-emerald-500/10 p-5">
            <div className="mb-5 flex items-center gap-3">
              <div className="rounded-2xl bg-emerald-500/15 p-3 text-emerald-300"><CheckCircle2 /></div>
              <div><h2 className="text-2xl font-black">Alta creada</h2><p className="text-sm text-emerald-100/70">Ya puedes continuar desde el PC o abrir el expediente.</p></div>
            </div>
            ...
            <button onClick={reset} className="mt-5 w-full rounded-2xl border border-white/10 px-5 py-4 font-black text-white">Crear otra alta</button>
          </section>
        )}
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-400">
          ...
          Añadiremos cámara/fotos directas para matrícula, vehículo, ECU, etiqueta y documentación.
        </div>
      </main>
...
function MobileField({ label, value, onChange, placeholder, icon, inputMode }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; icon?: React.ReactNode; inputMode?: React.HTMLAttributes<HTMLInputElement>['inputMode'] }) {
  return (
    <label className="mb-4 block">
