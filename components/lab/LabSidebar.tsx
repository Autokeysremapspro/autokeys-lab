    ...
    <>
      ...
      <aside
        className={[
          ...
          'shadow-[24px_0_60px_rgba(0,0,0,.22)] transition-transform duration-300 lg:sticky lg:top-0 lg:h-screen lg:w-[232px] lg:translate-x-0',
          open ? 'translate-x-0' : '-translate-x-full',
        ].join(' ')}
      >
        <div className="mb-5 flex h-[58px] items-center justify-between gap-2 px-1.5">
          <Link href="/" onClick={onClose} className="flex min-w-0 items-center gap-2">
            <LabLogoMark size={42} />
            <LabWordmark />
          </Link>
          <button onClick={onClose} className="rounded-lg border border-white/10 p-2 text-zinc-400 hover:bg-white/5 lg:hidden" aria-label="Cerrar menú">
            <X size={17} />
          </button>
        </div>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={onClose}
                className={[
                  'group flex min-h-[43px] items-center gap-3 rounded-lg border px-3 py-2.5 text-[13px] font-medium transition-all duration-150',
                  active
              ...
              >
                <Icon size={17} strokeWidth={1.8} className={active ? 'text-white' : 'text-zinc-500 group-hover:text-zinc-300'} />
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center gap-3 rounded-xl border border-white/[0.08] bg-[#0d0f13] px-3 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,.025)]">
            <LabProBadge size={35} />
          ...
          <div className="flex items-center gap-2.5 rounded-xl border border-white/[0.07] bg-[#0b0d11] px-3 py-3 text-xs">
