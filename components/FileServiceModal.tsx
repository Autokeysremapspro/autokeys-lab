    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-4xl rounded-3xl border border-white/10 bg-[#0B1220] p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-[#ff5468] font-bold">File Service</p>
        ...
        <label className="block space-y-2 mt-4">
          ...
          <textarea value={form.notas || ''} onChange={(e) => setField('notas', e.target.value)} rows={4} className="w-full rounded-2xl border border-white/10 bg-[#111827] px-4 py-3 outline-none focus:border-[#c81f2a]" placeholder="Observaciones del archivo, peticiones del taller, avisos..." />
        </label>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button disabled={saving} className="rounded-2xl bg-[#c81f2a] px-5 py-3 font-bold text-[#0a0d12] hover:bg-[#ff5468] disabled:opacity-60">
...
function Input({ label, value, onChange, type = 'text', required = false }: any) {
  return (
    <label className="space-y-2">
      ...
      <input required={required} type={type} value={value || ''} onChange={(e) => onChange(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-[#111827] px-4 py-3 outline-none focus:border-[#c81f2a]" />
    </label>
  )
}
