      ...
      <aside className="w-72 bg-[#0F172A] border-r border-white/10 p-5 hidden lg:flex flex-col">
        ...
        <nav className="space-y-1 flex-1">
          {nav.map(([href, label, Icon]: any) => {
            const active = pathname === href || (href !== '/' && pathname.startsWith(href) && ['Expedientes','Clientes','Vehículos','File Service','Stock','Facturas','Usuarios','Biblioteca Técnica','Informes','Configuración'].includes(label))
            return (
              <Link
                key={`${href}-${label}`}
                href={href}
